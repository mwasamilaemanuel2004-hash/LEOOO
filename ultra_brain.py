...
class PatternMemory:
    ...
    def predict(self, features: np.ndarray, k: int = 5) -> dict:
        """Find k most similar patterns and compute win probability."""
        if len(self.patterns) < 5:
            return {"confidence": 0.5, "direction": "unknown", "pattern_count": 0}

        feat_list = features[:20].tolist()
        similarities = []
        for p in self.patterns.values():
            if p.count >= 3:  # Only use patterns seen 3+ times
                sim = self._cosine_sim(feat_list, p.features)
        ...
        long_wr  = sum(s * p.win_rate for s, p in top_k if p.direction == "long") / (total_weight + 1e-9)
        short_wr = sum(s * p.win_rate for s, p in top_k if p.direction == "short") / (total_weight + 1e-9)
